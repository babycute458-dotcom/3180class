...
const SAMPLE = [
  ...
  },
  {
    ...
    pinyin: "/ˈskelɪtn/",
    pos: "名詞 n.",
    meaning: "骨骼、骨架；the bones that support a body",
    example: "The museum shows a dinosaur skeleton. 博物館展示了一具恐龍骨骼。",
    lesson: "單字清單",
    sentences: [],
    compounds: [],
    known: false,
  },
  {
    ...
    pinyin: "/ˌekskəˈveɪʃn/",
    pos: "名詞 n.",
    meaning: "挖掘、發掘；the act of digging to find something underground",
    example: "The excavation uncovered an ancient skull. 這次發掘挖出了一個古老的頭骨。",
    lesson: "單字清單",
    sentences: [],
    compounds: [],
    known: false,
  },
  {
    ...
    pinyin: "/ɡræb/",
    pos: "動詞 v.",
    meaning: "抓住、搶拿；to take something quickly with your hand",
    example: "Please grab the rope carefully. 請小心地抓住繩子。",
    lesson: "單字清單",
    sentences: [],
    compounds: [],
    known: false,
  },
  {
    ...
    pinyin: "/skʌl/",
    pos: "名詞 n.",
    meaning: "頭骨、頭蓋骨；the bones of the head",
    example: "The skull protects the brain. 頭骨保護大腦。",
    lesson: "單字清單",
    sentences: [],
    compounds: [],
    known: false,
  },
  {
    ...
    pinyin: "/ˈlæbrətɔːri/",
    pos: "名詞 n.",
    meaning: "實驗室；a room where scientific tests and research are done",
    example: "The scientists work in the laboratory. 科學家在實驗室工作。",
    lesson: "單字清單",
    sentences: [],
    compounds: [],
    known: false,
  },
  {
    ...
    pinyin: "/ˈbæləns/",
    pos: "名詞／動詞 n./v.",
    meaning: "平衡；使平衡；a steady position, or to keep something steady",
    example: "It is hard to keep your balance on ice. 在冰上保持平衡很難。",
    lesson: "單字清單",
    sentences: [],
    compounds: [],
    known: false,
  ...
  },
  {
    ...
    pinyin: "/fɔːrθ/",
    pos: "副詞 adv.",
    meaning: "向前、往外；out into view, or forward in place or time",
    example: "The plants put forth new leaves in spring. 植物在春天長出新葉。",
    lesson: "單字清單",
    sentences: [],
    compounds: [],
    known: false,
...
function loadWords() {
  try {
    ...
    const existing = new Set(
      parsed.map((w) => String(w.word || "").trim().toLowerCase()).filter(Boolean)
    );
    const merged = [...parsed];
    for (const word of sample) {
      if (!existing.has(word.word.toLowerCase())) merged.push({ ...word });
    }
    return merged;
  } catch {
...
function sentenceHtml() {
  ...
  return `
    ...
    <section class="panel">
      ...
      <div id="draftFeedback">${state.draftFeedback ? feedbackHtml(state.draftFeedback) : ""}</div>
    ...
    .map(([author, items]) => {
      ...
      return `<article class="author-card ${author === me ? "mine" : ""}">
        <div class="author-head">
          ...
          <span class="meta">共 ${items.length} 句</span>
        </div>
        ${ordered.map((item, idx) => sentenceBlockHtml(item, me, idx === 0)).join("")}
      </article>`;
    })
    .join("");
}
...
function sentenceBlockHtml(item, me, isLatest) {
  ...
  return `<div class="sentence-block ${isLatest ? "is-latest" : ""}" data-sentence-id="${item.id}">
    <p class="${isLatest ? "latest" : "history-line"}">
      ...
      ${item.revisionOf ? `<span class="chip pine">修改版</span>` : ""}
    ...
    ${
      (fb.notes || []).length
        ? `<ul class="feedback-notes">${fb.notes
            .map(
              (note) =>
                `<li>${escapeHtml(note.shortMessage || note.message)}${
                  note.replacements?.length
                    ? ` → ${note.replacements.map((r) => escapeHtml(r)).join(" / ")}`
                    : ""
                }</li>`
...
async function postSentenceEntry(text, extra = {}) {
  const name = Classroom.displayName();
  const word = currentWord("sentenceIndex");
  if (!text || !name || !Classroom.roomId || !word) return;
